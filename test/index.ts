...
import { createDocumentFromQuery } from "../src/extractFromAST";

import { ExtractGQL } from "../src/ExtractGQL";

import { OutputMap } from "../src/common";

import { addTypenameTransformer } from "../src/queryTransformers";

import { parse, print, OperationDefinitionNode, DocumentNode } from "graphql";
...
describe("ExtractGQL", () => {
  ...
  describe("isDirectory", () => {
    ...
    it("should return false on a file", (done) => {
      ExtractGQL.isDirectory(
        "./test/fixtures/single_query/queries.graphql"
      ).then((result) => {
        assert(!result);
        done();
      });
  ...
  describe("getFileExtension", () => {
    it("should return the correct extension on a file with an extension", () => {
      assert.equal(
        ExtractGQL.getFileExtension("../../path/source.graphql"),
        "graphql"
      );
      assert.equal(
        ExtractGQL.getFileExtension(
          "/some/complicated/path.with.dots/dots../view.js"
        ),
        "js"
      );
    });
  ...
  describe("createMapFromDocument", () => {
    ...
    it("should sort fragments correctly", () => {
      const myegql = new ExtractGQL({ inputFilePath: "empty" });
      const doc = gql`
        fragment d on Author {
          x
        }
        fragment b on Author {
          x
        }
        fragment c on Author {
          x
        }
  ...
  describe("processGraphQLFile", () => {
    it("should be able to load a GraphQL file with multiple queries", (done) => {
      egql
        .processGraphQLFile("./test/fixtures/single_query/queries.graphql")
        .then((documentMap) => {
  ...
  describe("readInputFile", () => {
    ...
    it("should correctly process a file with a .graphql extension", (done) => {
      egql
        .readInputFile("./test/fixtures/single_query/queries.graphql")
        .then((result: string) => {
  ...
  describe("processInputPath", () => {
    it("should process a single file", (done) => {
      egql
        .processInputPath("./test/fixtures/single_query/queries.graphql")
    ...
    it("should process a directory with a single file", (done) => {
      egql
        .processInputPath("./test/fixtures/single_query")
        .then((result: OutputMap) => {
          assert.equal(Object.keys(result).length, 2);
          assert.include(
            Object.keys(result),
    ...
    it("should process a directory with no graphql files", (done) => {
      egql
        .processInputPath("./test/fixtures/no_queries")
        .then((result: OutputMap) => {
          assert.equal(Object.keys(result).length, 2);
          done();
        });
    ...
    it("should process a file with a fragment reference to a different file", () => {
      ...
      return egql
        .processInputPath("./test/fixtures/single_fragment")
        .then((result: OutputMap) => {
          const keys = Object.keys(result);
          assert.equal(keys.length, 1);
          assert.include(Object.keys(result), print(expectedQuery));
        });
    });
    ...
    it("should process a JS file with queries", () => {
      ...
      return jsEgql
        .processInputPath("./test/fixtures/single_fragment_js")
        .then((result: OutputMap) => {
          const keys = Object.keys(result);
          assert.equal(keys.length, 1);
          assert.equal(keys[0], print(expectedQuery));
        });
    });
  });
  ...
  describe("writeOutputMap", () => {
    it("should be able to write an OutputMap to a file", (done) => {
      ...
      egql
        .writeOutputMap(outputMap, "./test/output_tests/output.graphql")
        .then(() => {
          done();
        })
        .catch((err) => {
